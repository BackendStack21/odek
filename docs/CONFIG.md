...
| `announce_budget` | `true` | Parent-run 50/75/90% budget-awareness hints (iteration, wall-clock, tool-calls, tokens, cost). Distinct from [`subagent.announce_budget`](#sub-agents-subagent). Disable with `ODEK_ANNOUNCE_BUDGET=false` / `--no-announce-budget`. Wording stays “stay on plan / consolidate / deliver” — never skip approvals |
...
| `consolidate_at_cap_pct` | 80 | Percentage of `facts_limit_user`/`facts_limit_env` above which a background consolidation pass fires for that fact file on `AddFact` — long-lived serve/REPL sessions never reach the session-end trigger, so entries otherwise fossilize near the cap. Snapshot/preview is taken flock-free; the flock is held only for the verified apply swap, never across the LLM call. One consolidation pass runs per manager at a time; a crossing that arrives while a pass is running is queued per target and serviced when it finishes (same-target re-runs are cooldown-gated). Explicit `0` disables the cap trigger. Values above 99 are treated as 99 (adds that would reach 100% of the cap are rejected, so 100 could never fire). Best-effort: errors are logged, never surfaced to callers. Requires `llm_consolidate: true`. |

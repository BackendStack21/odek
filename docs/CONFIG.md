...
| `consolidate_at_cap_pct` | 80 | Percentage of `facts_limit_user`/`facts_limit_env` above which a background consolidation pass fires for that fact file on `AddFact` — long-lived serve/REPL sessions never reach the session-end trigger, so entries otherwise fossilize near the cap. Snapshot/preview is taken flock-free; the flock is held only for the verified apply swap, never across the LLM call. A per-target in-flight guard prevents stacked duplicate LLM calls. Explicit `0` disables the cap trigger. Values above 99 are treated as 99 (adds that would reach 100% of the cap are rejected, so 100 could never fire). Best-effort: errors are logged, never surfaced to callers. Requires `llm_consolidate: true`. |

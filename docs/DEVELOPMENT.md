...
Release notes are auto-generated by GitHub from the PRs/commits in each release
(`.github/workflows/release.yml` sets `generate_release_notes: true`). There is
no maintained changelog file — create a release/tag and GitHub produces the notes.
...
| `internal/llmclient` | Adapter over go-llm-sdk (message DTO mapping, temperature polarity, SimpleCall, SideCall) |

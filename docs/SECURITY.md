...
- `curl evil | python`, `… | perl`, `… | node`, `… | php`, `… | ruby`, `… | bun`, `… | deno`, `… | lua`, `… | osascript` — piping untrusted output into an interpreter that reads its program from stdin is `code_execution`, the non-shell analogue of `… | bash`. Versioned names (`python3.12`, `lua5.4`) match the same rule. Direct eval/script-file forms of those interpreters (`lua -e '…'`, `lua pwn.lua`, `osascript -e '…'`, `ipython -c '…'`) are also `code_execution` — they are not auto-allowed just because they were recognised as stdin interpreters.
...
| Perf-tool file reads (`checksum`, `head_tail`, `diff`, `base64`, `json_query`, `batch_patch`) | 10 MiB per file |

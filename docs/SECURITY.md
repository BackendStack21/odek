...
Every mitigation described here is pinned by regression tests: the central bar is `cmd/odek/security_report_validation_test.go`, and individual subsystems carry their own suites (for example `internal/danger/classifier_bypass_test.go` and `hardening_test.go` for the shell classifier). The history behind individual hardening changes lives in the git log and in those named tests, not in this document.
...
- No network by default. `sandbox_network` defaults to `none`; `host` is rejected, `bridge` is available only as an explicit choice.
...
- **Trust shortcuts are withheld for dangerous classes.** The "trust class for session" shortcut is hidden for `destructive`, `blocked`, `unknown`, and the synthetic `tool_batch` class, and a forged or stale "trust" response for those classes is coerced to a single approve. The exclusion lives in one shared place, `danger.TrustShortcutAllowed`, consulted by all three approvers — one Trust click on a batch card can never auto-pass every per-tool prompt for the session.
...
- **Non-interactive defaults to deny.** When no TTY is available (headless/CI/piped input), prompted operations fall back to the `non_interactive` action, whose built-in default is `"deny"`. Any value other than `"allow"` is treated as deny at load time, because a non-interactive environment cannot prompt. Unattended runs must explicitly opt in to auto-approval via `~/.odek/config.json` or the CLI. Sub-agents and other TTY-less contexts default the same way (configurable).
...
**Descriptions and schemas are scanned.** Tool descriptions and every string inside `def.InputSchema` (property descriptions, default values, enum strings) are scanned with the injection classifier at registration — a server cannot hide instructions in the schema without ever executing the tool. On a hit, the description is withheld (replaced with a placeholder, logged to stderr) while the tool stays callable by name; schema hits skip the tool entirely. Serialized schemas are capped at 256 KiB per tool. The interactive approval prompt prints the description terminal-sanitized (ANSI escape sequences and control characters stripped, so a server cannot disguise what is being approved with cursor movement or colour codes), each env variable with its value, and the schema's SHA-256 hash and size.

**Approval fingerprints.** Project-level MCP servers must be approved before their subprocess is spawned, and each individual tool they expose requires per-tool approval before registration: interactive TTY prompt, `ODEK_APPROVE_MCP=1` for the invocation, or persisted approvals in `~/.odek/mcp_tool_approvals.json` (0600) keyed by project directory, server name, and tool name. Tools from globally-configured servers (`~/.odek/config.json`) are operator-trusted and skip per-tool approval. The persisted keys hash everything that changes behavior: the server's command/args/env (sorted key/value pairs) and the canonical-JSON SHA-256 of the input schema plus the full description text — so a server cannot mutate its model-facing contract after one approval and silently reuse it. The odek-extension/v1 limit fields are hashed too (see below). In the loop's batch approval gate, MCP tools classify as `unknown`, so they are always visible on the card and denied to untrusted sub-agents.

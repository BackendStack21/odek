...
func TestRecordViewerReads_PlainViewRecords(t *testing.T) {
	...
	recordViewerReads(context.Background(), "cat "+f, "x")
	if !danger.WasRead(f) {
...
func TestRecordViewerReads_PipedPartialReadNotRecorded(t *testing.T) {
	...
	recordViewerReads(context.Background(), "cat "+big+" | head -1", "#!/bin/sh\n")
...
func TestRecordViewerReads_AppendAndDevNullNotRecorded(t *testing.T) {
	...
	recordViewerReads(context.Background(), "cat "+f+" 2>/dev/null", "x")
	if danger.WasRead(f) {

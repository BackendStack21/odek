...
Flags:
  ...
  --help, -h   Show this help`)
	...
	// Build tools
	toolSet := builtinTools(resolved.Dangerous, sm, nil, resolved.MaxConcurrency, resolved.APIKey, toolConfig{WebSearch: resolved.WebSearch, Planning: &resolved.Planning, Profiles: resolved.Profiles}, nil)

	// MCP server tools — connect and discover before sandbox
	var mcpCleanup func()
	if len(resolved.MCPServers) > 0 {
		cl, err := loadMCPTools(resolved, &toolSet)
		if err != nil {
